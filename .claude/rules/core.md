...
- `types.ts` — All domain types defined as Zod schemas with inferred TypeScript types. Zod schemas and TS types share the same name (e.g., `DecisionResult` is both). Key entity types: `UserFrontmatter` (humans who auth), `OrganizationFrontmatter` (legal entities), `OrgMembership` (join table with `OrgRole`: `org_admin | org_member | org_viewer`), `AgentFrontmatter` (polymorphic ownership via `owner_type: 'user' | 'org'` + `owner_id`). System RBAC: `SystemRole` (`'admin'`), `resolveSystemRoles()` helper. Session: `SessionClaims` includes `system_roles` and `org_memberships` (array of `{ org_id, role }`).
...
- `server-plugin.ts` — `ServerPluginContext`, `ServerPluginManifest`, `CreateServerPlugin`, `OrgVerificationProvider` (plugin interface for org verification via external registries). Manifest supports `extraHeadHtml`/`extraBodyHtml` for injecting HTML into owner pages, and `verificationProviders` for org verification plugins.
- `identity.ts` — Identity validation: `GovernmentId` (users), `CompanyId` (orgs), `ContactIdentity` (both), `Signatory`/`SignatoryRule` (orgs). Validation functions: `validateUserIdentity()`, `validateOrgIdentity()`, `computeUserAssuranceLevel()`, `computeOrgAssuranceLevel()`.
