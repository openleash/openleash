...
function scopeInitials(name: string): string {
...
function scopeAvatarHtml(scope: ScopeOption, variant: "button" | "item"): string {
    const cls = variant === "button" ? "scope-switcher-avatar" : "scope-item-avatar";
    const typeCls = scope.type === "org" ? " is-org" : " is-user";
    return `<span class="${cls}${typeCls}" aria-hidden="true">${escapeHtml(scopeInitials(scope.display_name))}</span>`;
}
...
function renderScopeSwitcher(scope: ScopeContext): string {
    ...
    return `<div class="scope-switcher" data-scope-switcher>
      <button type="button" class="scope-switcher-button" data-scope-switcher-toggle aria-haspopup="menu" aria-expanded="false">
        ${scopeAvatarHtml(current, "button")}
        <span class="scope-switcher-labels">
          <span class="scope-switcher-label">${currentLabel}</span>
          <span class="scope-switcher-sub">${currentSub}</span>
        </span>
        <span class="material-symbols-outlined scope-switcher-caret">unfold_more</span>
      </button>
      <div class="scope-switcher-menu" role="menu">
        <div class="scope-switcher-menu-label">Switch workspace</div>
        ...
        <div class="scope-switcher-menu-section">
          ...
          <a href="/gui/orgs" class="scope-item scope-item-action" role="menuitem">
            <span class="material-symbols-outlined scope-item-icon">tune</span>
            <span class="scope-item-labels">
...
export function renderPage(
    ...
    const dashboardPath = isOwner
        ? (ownerNavBase[0]?.path ?? "/gui/personal/dashboard")
        : "/gui/admin/dashboard";
    ...
    const showSwitcher =
        options?.isAdmin === true || (!isOwner && options?.showContextSwitcher !== false);
