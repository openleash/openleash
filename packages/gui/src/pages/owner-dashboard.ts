...
export function renderOwnerDashboard(data: OwnerDashboardData): string {
    ...
    <div class="card-grid" style="display:grid;grid-template-columns:repeat(auto-fill,minmax(200px,1fr));gap:16px;margin-bottom:24px">
      <div class="card" style="padding:20px">
        <div style="font-size:28px;font-weight:700;color:var(--color-success)">${data.agent_count}</div>
        <div style="color:var(--text-secondary);font-size:13px;margin-top:4px">My Agents</div>
      </div>
      <div class="card" style="padding:20px">
        <div style="font-size:28px;font-weight:700;color:var(--color-success)">${data.policy_count}</div>
        <div style="color:var(--text-secondary);font-size:13px;margin-top:4px">My Policies</div>
      </div>
      <div class="card" style="padding:20px">
        <div style="font-size:28px;font-weight:700;color:${data.pending_approvals > 0 ? "var(--color-warning)" : "var(--color-success)"}">${data.pending_approvals}</div>
        <div style="color:var(--text-secondary);font-size:13px;margin-top:4px">Pending Approvals</div>
      </div>
      <div class="card" style="padding:20px">
        <div style="font-size:28px;font-weight:700;color:${data.pending_policy_drafts > 0 ? "var(--color-warning)" : "var(--color-success)"}">${data.pending_policy_drafts}</div>
    ...
    ${
        data.pending_approvals > 0
            ? `
    <div class="card" style="padding:16px;margin-bottom:24px;border-left:3px solid var(--color-warning)">
      <span style="color:var(--color-warning)">You have ${data.pending_approvals} pending approval request${data.pending_approvals > 1 ? "s" : ""}.</span>
    ...
    ${
        data.pending_policy_drafts > 0
            ? `
    <div class="card" style="padding:16px;margin-bottom:24px;border-left:3px solid var(--color-warning)">
      <span style="color:var(--color-warning)">You have ${data.pending_policy_drafts} pending policy draft${data.pending_policy_drafts > 1 ? "s" : ""} from your agents.</span>
