...
export function renderAdminOrganizations(orgs: OrgListData[]): string {
    const rows = orgs.map((o) => {
        if (o.error) {
            return `<tr><td>${escapeHtml(o.org_id.slice(0, 8))}${idBadge(o.org_id)}</td><td colspan="4" class="text-muted">File not found</td></tr>`;
        }
        return `<tr>
      <td><a href="/gui/admin/organizations/${escapeHtml(o.org_id)}" class="table-link">${escapeHtml(o.display_name || "—")}</a>${idBadge(o.org_id)}</td>
...
export function renderAdminOrganizationDetail(data: OrgDetailData): string {
    ...
    const memberRows = members.map((m) => `<tr>
      <td>${escapeHtml(m.display_name || "—")}${idBadge(m.user_principal_id)}</td>
    ...
    const policyRows = policies.map((p) => {
        ...
        return `<tr>
      <td><a href="/gui/admin/policies/${escapeHtml(p.policy_id)}" class="table-link">${escapeHtml(p.name || "—")}</a>${idBadge(p.policy_id)}</td>
      <td>${appliesTo}</td>
    </tr>`;
    }).join("\n");
    ...
    <div class="card">
      ...
      ${agents.length === 0
        ...
        : `<table><thead><tr><th>Agent ID</th><th>Status</th><th>Created</th></tr></thead><tbody>${agentRows}</tbody></table>`}
    </div>
    ...
    <div class="card">
      ...
      ${policies.length === 0
        ...
        : `<table><thead><tr><th>Name</th><th>Applies to</th></tr></thead><tbody>${policyRows}</tbody></table>`}
    </div>
