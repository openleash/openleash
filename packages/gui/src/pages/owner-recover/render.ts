...
export function renderOwnerRecover(): string {
    return `<!DOCTYPE html>
<html lang="en">
<head>
  <meta charset="UTF-8">
  <meta name="viewport" content="width=device-width, initial-scale=1.0">
  <title>Recover Passphrase - OpenLeash</title>
  ...
  ${assetTags("pages/owner-recover/client.ts")}
</head>
<body>
  ...
  <div class="login-card">
    ${authBrandHtml("Recover Passphrase")}
    <form id="recover-form">
      <p class="orec-intro">Enter the email address on file for your account. If we find a match, we'll send a link you can use to set a new passphrase.</p>
      <div class="form-group">
        <label>Email</label>
        <input type="email" id="email" placeholder="you@example.com" autocomplete="email" required>
      </div>
      <button type="submit" class="btn-login" id="submit-btn">Send recovery link</button>
      <div class="error-msg" id="error-msg"></div>
    </form>
    <div id="success-msg" class="success-msg orec-hidden">
      <h2>Check your email</h2>
      <p>If an account exists for that address, a recovery link has been sent. The link expires in 24 hours.</p>
      <p><a href="/gui/login">Back to sign in</a></p>
    </div>
    <div class="admin-link orec-back">
      <a href="/gui/login">Back to sign in</a>
    </div>
  </div>
</body>
</html>`;
