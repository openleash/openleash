...
<body>
  ...
  <script>
    ...
    function olApiError(data,fallback){if(!data||!data.error)return fallback||'An error occurred';var e=data.error;if(e.code==='VALIDATION_ERROR'&&e.field_errors){var fe=e.field_errors;var keys=Object.keys(fe);for(var i=0;i<keys.length;i++){olFieldError(keys[i],fe[keys[i]]);}var msgs=[];for(var i=0;i<keys.length;i++){msgs.push(fe[keys[i]]);}return msgs.join('; ');}return e.message||fallback||'An error occurred';}
    var _olResolve=null;
    var _olDialogValidator=null;
    function _olDialogReset(){olFieldError('ol-dialog-input','');var inp=document.getElementById('ol-dialog-input');inp.removeAttribute('maxlength');inp.removeAttribute('inputmode');inp.oninput=null;_olDialogValidator=null;}
    function olDialogCancel(){_olDialogReset();document.getElementById('ol-dialog').classList.remove('open');if(_olResolve){var fn=_olResolve;_olResolve=null;fn(null);}}
    async function olDialogOk(){var wrap=document.getElementById('ol-dialog-input-wrap');var val=wrap.style.display!=='none'?document.getElementById('ol-dialog-input').value:true;if(_olDialogValidator){var btn=document.getElementById('ol-dialog-ok');var orig=btn.textContent;btn.disabled=true;btn.textContent='Verifying\u2026';var err;try{err=await _olDialogValidator(val);}catch(e){err=e.message||'An error occurred';}btn.disabled=false;btn.textContent=orig;if(err){olFieldError('ol-dialog-input',err);return;}}document.getElementById('ol-dialog').classList.remove('open');_olDialogReset();if(_olResolve){var fn=_olResolve;_olResolve=null;fn(val);}}
    ...
    function ol2FA(onSubmit){return new Promise(function(r){_olResolve=function(v){r(v===null?null:v.replace(/\\s/g,''));};document.getElementById('ol-dialog-title').textContent='Two-Factor Authentication';document.getElementById('ol-dialog-msg').textContent='Enter your 2FA code:';var inp=document.getElementById('ol-dialog-input');inp.value='';inp.placeholder='000 000';inp.setAttribute('inputmode','numeric');inp.setAttribute('maxlength','7');inp.oninput=function(){var raw=this.value.replace(/[^0-9]/g,'').slice(0,6);var formatted=raw.length>3?raw.slice(0,3)+' '+raw.slice(3):raw;if(this.value!==formatted)this.value=formatted;olFieldError('ol-dialog-input','');};document.getElementById('ol-dialog-input-wrap').style.display='block';document.getElementById('ol-dialog-cancel').style.display='';document.getElementById('ol-dialog-ok').textContent='Confirm';_olDialogValidator=function(v){var digits=v.replace(/\\s/g,'');if(!/^\\d{6}$/.test(digits))return 'Code must be exactly 6 digits';if(onSubmit)return onSubmit(digits);return null;};document.getElementById('ol-dialog').classList.add('open');inp.focus();});}
  </script>
